...
class RKIntegration:
    ...
    supported_methods: List[str] = ["rk1", "rk2", "rk4"]
    ...
    def integrate(self, u0: np.array, t: float, dt: float, de_solver: DESolver) -> np.array:
        ...
        if self.method == "rk1":
            # RK1 (Forward Euler)
            return u + dt * de_solver.du_dt(u, t)

        elif self.method == "rk2":
            # RK2: Heun's method
            k1 = de_solver.du_dt(u, t)
            k2 = de_solver.du_dt(u + 0.5 * dt * k1, t + 0.5 * dt)
            return u + dt * k2
            ...
            k1 = de_solver.du_dt(u, t)
            k2 = de_solver.du_dt(u + 0.5 * dt * k1, t + 0.5 * dt)
            k3 = de_solver.du_dt(u + 0.5 * dt * k2, t + 0.5 * dt)
            k4 = de_solver.du_dt(u + dt * k3, t + dt)
